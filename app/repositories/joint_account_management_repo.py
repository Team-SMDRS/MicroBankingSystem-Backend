...
class JointAccountManagementRepository:
	def create_joint_account_with_existing_and_new_customer(self, existing_nic, new_customer_data, account_data, created_by_user_id):
		...
		try:
			...
			self.cursor.execute(
				...
				VALUES (%s, %s, %s, %s, %s)
				""",
				(new_customer_id, username, password, created_by_user_id, created_by_user_id)
			)
	...
	def _generate_username(self, full_name):
		...
		suffix = ''.join(self.random.choices(self.string.digits, k=4))
		return f"{base}{suffix}"

	def _generate_password(self, length=8):
		chars = self.string.ascii_letters + self.string.digits
	...
	def create_joint_account_with_new_customers(self, customer1_data, customer2_data, account_data, created_by_user_id):
		"""
		Create two new customers, auto-create their logins (NIC as username/password), create joint account, link both as owners.
		Returns: (customer1_id, customer2_id, acc_id, account_no)
		"""
		try:
			# Insert customer 1
			self.cursor.execute(
			...
			self.cursor.execute(
				...
				VALUES (%s, %s, %s, %s, %s)
				""",
				(customer1_id, username1, password1, created_by_user_id, created_by_user_id)
			)
			self.cursor.execute(
				"""
				INSERT INTO customer_login (customer_id, username, password, created_by, updated_by)
				VALUES (%s, %s, %s, %s, %s)
				""",
				(customer2_id, username2, password2, created_by_user_id, created_by_user_id)
			)
	...
	def create_joint_account(self, account_data, nic1, nic2, created_by_user_id):
		...
		try:
			...
			# Get branch of the user creating the account from users_branch table (user_id as UUID)
			self.cursor.execute("SELECT branch_id FROM users_branch WHERE user_id = %s", (created_by_user_id,))
			branch_row = self.cursor.fetchone()
			if not branch_row:
				return None
