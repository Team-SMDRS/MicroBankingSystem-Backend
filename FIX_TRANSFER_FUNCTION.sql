-- FIX: Update the transfer function to properly record transactions in history
-- Run this SQL to fix the transfer transaction recording issue
...
BEGIN
    ...
    BEGIN
        ...
        VALUES (gen_random_uuid(), -p_amount, p_from_acc_id, 'Bank Transfer', p_description, new_reference_no, NOW(), p_created_by);
        
        -- Insert bank transfer transaction for destination account (credits to destination)
        INSERT INTO transactions (transaction_id, amount, acc_id, type, description, reference_no, created_at, created_by)
        VALUES (gen_random_uuid(), p_amount, p_to_acc_id, 'Bank Transfer', p_description, new_reference_no, NOW(), p_created_by);
...
-- After running this, test your transfer and then check transaction history with:
-- SELECT * FROM transactions WHERE reference_no = YOUR_REFERENCE_NUMBER ORDER BY created_at;